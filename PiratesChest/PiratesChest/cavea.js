class cavea extends Phaser.Scene {
  ...
  preload() {
  //step 1 load Json
      this.load.tilemapTiledJSON("cavea","assets/cavea.json");
  ...
  create() {
      console.log('*** cavea scene');

      let map = this.make.tilemap({key: "cavea"});
...
// //collect item and 它的位子 然后移除
  ...
  this.co = this.physics.add.sprite(532,511,"co").play("co").setScale(0.3);
  //overlap_item1
...
//   this.physics.add.overlap(this.player, this.item2, this.collectitem2, null, this);
  ...
  update() {
    ...
    // go to world
  if (this.player.x > 436 && this.player.x < 500 && this.player.y > 116 && this.player.y < 139 ) {
    this.world();
  }
