class caveb extends Phaser.Scene {
  ...
  preload() {
  //step 1 load Json
      this.load.tilemapTiledJSON("caveb","assets/caveb.json");
  ...
  create() {
      console.log('*** caveb scene');

      let map = this.make.tilemap({key: "caveb"});
...
// //collect item and 它的位子 然后移除
  ...
  this.co = this.physics.add.sprite(52,587,"co").play("co").setScale(0.3);
  //overlap_item1
...
//   this.physics.add.overlap(this.player, this.item2, this.collectitem2, null, this);
  ...
  update() {
    ...
    if (this.player.x > 150 && this.player.x < 170 && this.player.y > 116 && this.player.y < 132 ) {
      this.world();
    }
